...
class InputTransition(nn.Module):
    ...
    def forward(self, x):
        ...
        x16 = torch.cat((x, x, x, x, x, x, x, x,
                         x, x, x, x, x, x, x, x), 1) # 채널을 더하는게 맞을거다.
...
class OutputTransition(nn.Module):
    ...
    def forward(self, x):
        ...
        # 기존엔 소프트맥스가 채널라스트 기준으로 작성되었나봄.
        # 채널라스트로 변경하지 않음, 자연스럽게 view 도 사용할 필요가 없음
        # ++++ 내 코드에서는 logit으,ㄹ 출력해야됨.
...
#         out = self.softmax(out)  # 소프트맥스 체널 지정.
